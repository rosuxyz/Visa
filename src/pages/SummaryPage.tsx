...
export function SummaryPage() {
  ...
  // feedbacks[i] = undefined → not yet started, null → failed, AIFeedback → done
  const [feedbacks, setFeedbacks] = useState<(AIFeedback | null | undefined)[]>(
    () => Array(completedAnswers.length).fill(undefined)
  );
  // Which question index is currently being analysed
  ...
  const fetchOverallReview = useCallback((results: (AIFeedback | null)[]) => {
    setOverallError('');
    ...
    // also update session score
    updateSessionScore(results);
  }, [completedAnswers, selectedType, updateSessionScore]);

  const backendSavedRef = useRef(false);

  // Sequential per-question analysis — one at a time so user sees each result appear
  useEffect(() => {
    ...
    const runSequential = async () => {
      ...
      for (let i = 0; i < completedAnswers.length; i++) {
        setAnalysingIdx(i);
        const qa = completedAnswers[i];
        const result = await getAIFeedback(qa.question.question, qa.transcript).catch(() => null);
        results.push(result);
        setFeedbacks(prev => {
          const next = [...prev];
          next[i] = result;
        ...
        // Small gap between questions to avoid rate limiting
        if (i < completedAnswers.length - 1) {
      ...
      // Fetch overall review only after all per-question analysis is done
      fetchOverallReview(results);
      ...
      if (!backendSavedRef.current) {
        ...
        const overallScore = scored.length
          ? Math.round(scored.reduce((sum, f) => {
  ...
  const scoredFeedbacks = feedbacks.filter((f): f is AIFeedback => f != null && f !== undefined);
  const avgScore = scoredFeedbacks.length
    ? Math.round(scoredFeedbacks.reduce((sum, f) => {
        const coh = f.coherenceScore ?? f.relevanceScore;
        return sum + (f.grammarScore + f.confidenceScore + f.relevanceScore + coh) / 4;
      }, 0) / scoredFeedbacks.length)
    : null;

  const doneCount = feedbacks.filter(f => f !== undefined).length;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
            ...
            <div className="bg-white rounded-xl border border-blue-100 p-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="flex justify-between text-xs font-semibold text-gray-600 mb-1.5">
                  <span>Analysing answers… ({doneCount}/{completedAnswers.length} done)</span>
                  <span>{Math.round((doneCount / completedAnswers.length) * 100)}%</span>
                </div>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-blue-500 rounded-full transition-all duration-500"
                    style={{ width: `${(doneCount / completedAnswers.length) * 100}%` }}
                  />
                </div>
              </div>
            </div>
        ...
        {/* ── Overall AI Review — shown only after all questions done ── */}
        {allDone && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-8">
            <div className="flex items-center gap-2 mb-5">
              <span className="text-xl">🤖</span>
              <h2 className="text-lg font-bold text-gray-900">Overall AI Review</h2>
              {overallLoading && <span className="w-4 h-4 rounded-full border-2 border-blue-400 border-t-transparent animate-spin ml-1" />}
            </div>

            {overallLoading && !overallReview && (
            ...
            {overallError && (
              <div className="flex items-start gap-3">
                ...
                <button onClick={() => fetchOverallReview(feedbacks.filter((f): f is AIFeedback => f != null && f !== undefined))}
        ...
        {/* ── Per-question cards ── */}
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-5">Question-by-Question Results</h2>
